...
<script>
    ...
    function live_jadwal() {
        ...
        //     // 8
        // } else
        if (selesai9.getTime() < mulai9.getTime()) {
            $('#error-jadwal9').show();
            $("#erorr_jadwal_row9").css("background-color", "red");
            $("#erorr_jadwal_row9").css("color", "white");
            $('.btnSave').attr('disabled', 'disabled');
...
<script>
    ...
    var form_jadwal_tender_penunjukan_langsung = $('#form_jadwal_tender_penunjukan_langsung');
    form_jadwal_tender_penunjukan_langsung.on('submit', function(e) {
        e.preventDefault();
        $.ajax({
            url: '<?= base_url('post_jadwal/post_jadwal/update_jadwal_19_tender_penunjukan_langsung/') ?>' + id_url_rup,
            method: "POST",
            ...
            success: function(response) {
                if (response == 'success') {
                    ...
                    form_jadwal_tender_penunjukan_langsung[0].reset();
                    $('.btnSave').attr('disabled', false);
                    setTimeout(() => {
...
<script>
    ...
    function plus_jadwal_22_baris(id_jadwal_rup) {
        ...
        $.ajax({
            type: "POST",
            url: '<?= base_url('post_jadwal/post_jadwal/genrate_jadwal_19_baris_plus/') ?>' + id_rup_global,
            data: {
    ...
    function min_jadwal_22_baris(id_jadwal_rup) {
        ...
        $.ajax({
            type: "POST",
            url: '<?= base_url('post_jadwal/post_jadwal/genrate_jadwal_19_baris_min/') ?>' + id_rup_global,
            data: {
                id_jadwal_rup: id_jadwal_rup
            },
            dataType: "JSON",
