...
<script>
    ...
    function live_jadwal() {
        ...
        // } 
        // else
        if (selesai8.getTime() < mulai8.getTime()) {
            $('#error-jadwal8').show();
            $("#erorr_jadwal_row8").css("background-color", "red");
            $("#erorr_jadwal_row8").css("color", "white");
            $('.btnSave').attr('disabled', 'disabled');
...
<script>
    ...
    form_jadwal_tender_terbatas.on('submit', function(e) {
        e.preventDefault();
        $.ajax({
            url: '<?= base_url('post_jadwal/post_jadwal/update_jadwal_21_tender_terbatas/') ?>' + id_url_rup,
            method: "POST",
...
<script>
    ...
    function plus_jadwal_21_baris(id_jadwal_rup) {
        var id_rup_global = $('[name="id_rup_global"]').val()
        $.ajax({
            type: "POST",
            url: '<?= base_url('post_jadwal/post_jadwal/genrate_jadwal_21_baris_plus/') ?>' + id_rup_global,
            data: {
    ...
    function min_jadwal_21_baris(id_jadwal_rup) {
        var id_rup_global = $('[name="id_rup_global"]').val()
        $.ajax({
            type: "POST",
            url: '<?= base_url('post_jadwal/post_jadwal/genrate_jadwal_21_baris_min/') ?>' + id_rup_global,
            data: {
                id_jadwal_rup: id_jadwal_rup
            },
            dataType: "JSON",
