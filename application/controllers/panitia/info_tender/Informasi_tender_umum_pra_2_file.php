...
class Informasi_tender_umum_pra_2_file extends CI_Controller
{
    ...
    public function informasi_pengadaan($id_url_rup)
    {
        ...
        $data['peserta_tender2'] = $this->M_panitia->get_peserta_tender_ba_pra_penawaran($data['row_rup']['id_rup']);
        // $data['peserta_tender_chat_penawaran'] = $this->M_panitia->get_peserta_tender_chat_penawaran($data['row_rup']['id_rup']);
        ...
        $data['peserta_tender_pq_penawaran_file1'] = $this->M_panitia->get_peserta_tender_ba_pra_penawaran_file1($data['row_rup']['id_rup']);
    ...
    public function evaluasi($id_url_rup)
    {
        ...
        $data['jadwal_evaluasi_dokumen_kualifikasi'] =  $this->M_jadwal->jadwal_pra_umum_6($data['row_rup']['id_rup']);
        $data['jadwal_presentasi_evaluasi'] =  $this->M_jadwal->jadwal_pra_umum_14($data['row_rup']['id_rup']);
        $data['pembukaan_penawaran_file_II'] =  $this->M_jadwal->jadwal_pra_umum_16($data['row_rup']['id_rup']);
        $this->load->view('template_tender/header');
        $this->load->view('panitia/info_tender/' . $root_jadwal . '/base_url_info_tender', $data);
    ...
    public function get_evaluasi_penawaran($id_rup)
    {
        ...
        $jadwal_pengumuman_pemenang =  $this->M_jadwal->jadwal_pra_umum_19($id_rup);

        $result = $this->M_panitia->gettable_evaluasi_penawaran($id_rup);
        $data = [];
        $no = $_POST['start'];
        foreach ($result as $rs) {
            $cek_menang = $this->M_panitia->cek_menang_umum($id_rup, $jadwal_pengumuman_pemenang, $rs->id_vendor);
                ...
                if ($rs->ev_penawaran_akhir >= 60) {
                    if ($rs->ev_penawaran_hps >= 100) {
                        ...
                        if ($rs->sts_pemenang_real == 1) {
                            $row[] = '<span class="badge bg-success bg-sm">Sah (Pemenang Sekarang)</span>';
                        } else {
                            if ($rs->ket_pindah_pemenang) {
                                $row[] = '<span class="badge bg-success bg-sm">' . $rs->ket_pindah_pemenang . '</span>';
                            } else {
            ...
            if (date('Y-m-d H:i', strtotime($jadwal['waktu_mulai']))  >= date('Y-m-d H:i')) {
                ...
                if ($rs->sts_pindah_pemenang == 1) {
                    if ($rs->sts_pemenang_real == 1) {
                        if ($total_menang >= 1) {
                            $row[] = '<div class="text-center"><a href="javascript:;" class="btn btn-info btn-sm shadow-lg text-white" onclick="byid_mengikuti(' . "'" . $rs->id_vendor_mengikuti_paket . "','penawaran'" . ')">
                                        <i class="fa-solid fa-edit"></i>
                                        <small>Evaluasi</small>
                                    </a>
                    ...
                    if ($rs->ev_penawaran_peringkat == 1) {
                        if ($total_menang >= 1) {
                            $row[] = '<div class="text-center"> 
                ...
                if ($rs->sts_pindah_pemenang == 1) {
                    if ($rs->sts_pemenang_real == 1) {
                        if ($total_menang >= 1) {
                            $row[] = '<div class="text-center"><button href="javascript:;" class="btn btn-secondary btn-sm shadow-lg text-white" disabled>
                        <i class="fa-solid fa-edit"></i>
                        <small>Evaluasi</small>
                        </button> 
                        <a href="javascript:;" class="btn btn-info btn-sm shadow-lg text-white" onclick="byid_mengikuti(' . "'" . $rs->id_vendor_mengikuti_paket . "','penawaran'" . ')">
    ...
    public function simpan_evaluasi_akhir_tkdn()
    {
        ...
        $id_rup_post = $this->input->post('id_rup_post');
        $ev_hea_penawaran = $row_vendor['nilai_penawaran'];
        ...
        if ($row_rup['bobot_nilai'] == 1) {
            ...
            if ($peserta_vendor >= 3) {
                if ($row_rup['sts_pindah_pemenang'] == 1) {
                    $get_rank1 = $this->M_panitia->get_peserta_rank1_pindah_pemenang($row_rup['id_rup']);
                    $message = 'Pengumuman Hasil ' . $row_rup['nama_metode_pengadaan'] . ' Pemenang untuk ' . $row_rup['nama_rup'] . ' adalah  ' . $get_rank1['nama_usaha'] . '  dengan Nilai penawaran sebesar Rp.' . number_format($get_rank1['nilai_penawaran'], 2, ',', '.') . 'Terima kasih atas ke-ikutsertaan Anda Ttd Panitia.';
                } else {
                ...
                $message = 'Pengumuman Hasil ' . $row_rup['nama_metode_pengadaan'] . ' Pemenang untuk ' . $row_rup['nama_rup'] . ' adalah  ' . $get_rank1['nama_usaha'] . '  dengan Nilai penawaran sebesar Rp.' . number_format($get_rank1['total_hasil_negosiasi'], 2, ',', '.') . 'Terima kasih atas ke-ikutsertaan Anda Ttd Panitia.';
            }
        } else {
            $peserta_vendor = $this->M_panitia->jumlah_peserta_negosiasi($row_rup['id_rup']);
            if ($peserta_vendor > 2) {
                $get_rank1 = $this->M_panitia->get_peserta_rank1_biaya($row_rup['id_rup']);
                $message = 'Pengumuman Hasil ' . $row_rup['nama_metode_pengadaan'] . ' Pemenang untuk ' . $row_rup['nama_rup'] . ' adalah  ' . $get_rank1['nama_usaha'] . '  dengan Nilai penawaran sebesar Rp.' . number_format($get_rank1['ev_terendah_harga'], 2, ',', '.') . 'Terima kasih atas ke-ikutsertaan Anda Ttd Panitia.';
            } else {
    ...
    public function aanwijzing_penawaran($id_url_rup)
    {
        ...
        $root_jadwal = $data['row_rup']['root_jadwal'];
        $data['root_jadwal'] = $data['row_rup']['root_jadwal'];
        $data['jadwal_aanwizing'] = $this->M_jadwal->jadwal_pra_umum_11($data['row_rup']['id_rup']);
        $data['data2'] = $this->M_tender->getDataById($data['row_rup']['id_rup']);
        $this->load->view('template_tender/header');
    ...
    public function get_vendor_negosiasi()
    {
        ...
        if ($row_rup['bobot_nilai'] == 1) {
            if ($row_rup['sts_pindah_pemenang'] == 1) {
                $result_vendor_negosiasi = $this->M_panitia->get_result_vendor_negosiasi_teknis_pindah($id_rup);
            } else {
    ...
    function summary_tender($id_url_rup)
    {
        ...
        // var_dump($data['result_vendor_terundang'][0]);
        // die;
    ...
    public function ba_pembuktian_kualifikasi($id_url_rup)
    {
        ...
        $data['peserta_tender'] = $this->M_panitia->get_peserta_tender($data['row_rup']['id_rup']);
        $data['panitia_tender'] = $this->M_panitia_terpilih->get_panitia_ba2($data['row_rup']['id_rup']);
        $data['get_syarat_tambahan'] = $this->M_panitia->result_syarat_tambahan($data['row_rup']['id_rup']);
    ...
    public function ba_hasil_evaluasi($id_url_rup)
    {
        ...
        $data['panitia_tender'] = $this->M_panitia_terpilih->get_panitia_ba3($data['row_rup']['id_rup']);
    ...
    public function ba_undangan_rapat($id_url_rup)
    {
        ...
        $data['peserta_tender'] = $this->M_panitia->get_peserta_tender($data['row_rup']['id_rup']);
        $data['peserta_tender_pq'] = $this->M_panitia->get_peserta_tender_ba_pra($data['row_rup']['id_rup']);
        $data['peserta_tender_pq_penawaran'] = $this->M_panitia->get_peserta_tender_ba_pra_penawaran_file1($data['row_rup']['id_rup']);
    ...
    public function ba_hasil_evaluasi_teknis($id_url_rup)
    {
        ...
        $data['peserta_tender_pq'] = $this->M_panitia->get_peserta_tender_ba_pra($data['row_rup']['id_rup']);
        $data['peserta_tender_pq_penawaran'] = $this->M_panitia->get_peserta_tender_ba_pra_penawaran($data['row_rup']['id_rup']);
        $data['get_peserta_tender_ba_pra_penawaran_file1'] = $this->M_panitia->get_peserta_tender_ba_pra_penawaran_file1($data['row_rup']['id_rup']);
        $data['panitia_tender'] = $this->M_panitia->get_panitia($data['row_rup']['id_rup']);
        $data['ba_teknis_detail'] = $this->M_panitia->get_ba_evaluasi_teknis($data['row_rup']['id_rup']);
        $data['panitia_tender'] = $this->M_panitia_terpilih->get_panitia_ba8($data['row_rup']['id_rup']);
    ...
    public function ba_sampul_II($id_url_rup)
    {
        ...
        $data['peserta_tender_pq'] = $this->M_panitia->get_peserta_tender_ba_pra($data['row_rup']['id_rup']);
        $data['peserta_tender_pq_penawaran'] = $this->M_panitia->get_peserta_tender_ba_pra_penawaran_file1($data['row_rup']['id_rup']);
        $data['panitia_tender'] = $this->M_panitia_terpilih->get_panitia_ba10($data['row_rup']['id_rup']);
    ...
    public function ba_evaluasinegosiasi($id_url_rup)
    {
        ...
        if ($data['row_rup']['id_jadwal_tender'] == 1 || $data['row_rup']['id_jadwal_tender'] == 9 || $data['row_rup']['id_jadwal_tender'] == 10) {
            $data['deal'] = $this->M_panitia->deal($data['row_rup']['id_rup']);
            $data['deal_row'] = $this->M_panitia->deal_row($data['row_rup']['id_rup']);
        } else {
            $data['deal'] = $this->M_panitia->deal_umum($data['row_rup']['id_rup']);
            $data['deal_row'] = $this->M_panitia->deal_row_umum($data['row_rup']['id_rup']);
        }
    ...
    public function ba_pemenang_tender($id_url_rup)
    {
        ...
        $data['get_rank1_tanpa_nego'] = $this->M_panitia->deal_row_umum($data['row_rup']['id_rup']);
    ...
    public function ba_penjelasan_kualifiaksi($id_url_rup)
    {
        ...
        $data['panitia_tender'] = $this->M_panitia_terpilih->get_panitia_ba1($data['row_rup']['id_rup']);
...
Terimakasih';
        ...
        } else if ($post == 'sts_kirim_undangan_presentasi_teknis') {
            $message = 'Pengumuman/Undangan Rapat Presentasi Teknis :
Nama Paket: ' . $row_rup['nama_rup'] . '
Status : Telah di Umumkan
Silahkan Login Melalui Link Ini untuk  melihat pengumuman/undangan : https://drtproc.jmto.co.id/auth 
Terimakasih';
            $this->kirim_wa->kirim_wa_pengumuman_atau_undangan_lolos_penawaran($row_rup['id_rup'], $message);
        } else if ($post == 'sts_kirim_pengumuman_hasil_evaluasi_teknis') {
            $message = 'Pengumuman Hasil Evaluasi Teknis :
Nama Paket: ' . $row_rup['nama_rup'] . '
Status : Telah di Umumkan
Silahkan Login Melalui Link Ini untuk  melihat pengumuman/undangan : https://drtproc.jmto.co.id/auth 
Terimakasih';
    ...
    {
        $ev_hea_tkdn = $this->uri->segment(6);
        $data = [
            'ev_hea_tkdn' => $ev_hea_tkdn,
            'ev_hea_harga' => 0
        ];
    ...
    public function pindahkan_pemenang()
    {
        ...
        $where2 = [
            'id_rup' => $id_rup,
            'ev_penawaran_peringkat'  => $ev_penawaran_peringkat + 1
        ];
        $this->M_panitia->update_evaluasi($data2, $where2);
